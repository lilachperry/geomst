...
class HTML:
    ...
    def add_images(self, epoch, ims, txts, links, width=400):
        ...
        if len(intr_inds) > 0:
            ...
            #bla = [m for l in txts for m in [regex.search(l)] if m]
            #txts2 = filter(regex.match, txts)
            gifname0 = '_'.join(tmp2[0:2]) + '.gif'
            gifname = os.path.join(img_dir_epoch, gifname0)
            #self.gen_gif(img_dir_epoch, ims[2:], gifname)
            self.gen_gif(img_dir_epoch, ims_for_gif, gifname)
            #bla = 1/0
            #gifname1 = '_'.join(tmp2[0:2]) + '_stn.gif'
            #gifname = os.path.join(img_dir_epoch, gifname1)
            #self.gen_gif(img_dir_epoch, ims[0:4], gifname)
            #ims.append(gifname0)
            #txts.append('gif')
            #links.append(gifname)
            ims = [gifname0] + ims
            txts = ['gif'] + txts
            links = [gifname0] + links
        #ims = [gifname0] + [gifname1] + ims
        #txts = ['gif'] + ['stn'] + txts
        #links = [gifname0] + [gifname1] + links
    ...
    def add_epoch_link(self, str, epoch):
        with self.doc:
            #h3(str)
            nm = 'epoch %d' % epoch
            a(nm, href=str, style="font-size:50px")#os.path.join(rel_path, link))
            br()
    ...
    def gen_gif(self, imdir, ims, outname):
        ...
        for idx, fname in enumerate(ims):
            ...
            #shp = im.shape
            #im = transform.resize(im, [0.5*shp[0], 0.5*shp[1], shp[2]])
            #im = sc.misc.imresize(im, interp='bicubic', size=0.5)
            images.append(im)
