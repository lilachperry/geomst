    ...
    def display_current_results(self, visuals, epoch, itnum, save_result):
        if self.display_id > 0:  # show images in the browser
            ...
            if ncols > 0:
                ...
                images = []
                idx = 0
                for label, image0 in vis_items:    #visuals.items():
                    regex = re.compile(".*(mp).*")
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            ...
            for label, image in visuals.items():
                ...
                #print("image tensor size: ", im.size())
                image_numpy = util.tensor2im(im)#image[0])
                tm = image[1]
            ...
            for label, image_numpy in visuals.items():
                ...
                tm = image_numpy[1]
                fname = image_numpy[2]
                img_path = 'epoch%.3d_%.4d_%s_%.3f.png' % (epoch, itnum, label, tm)  #'epoch%.3d_%s.png' % (n, label)
                ims.append(img_path)
                if fname != '':
                    label = label + " " + fname
                txts.append(label)
            ...
            #         # image_numpy = util.tensor2im(image[0])
            #         tm = image_numpy[1]
            #         img_path = 'epoch%.3d_%.4d_%s_%.3f.png' % (
            #         epoch, itnum, label, tm)  # 'epoch%.3d_%s.png' % (n, label)
            #         ims.append(img_path)
            #         txts.append(label)
            #         links.append(img_path)
